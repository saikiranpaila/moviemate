...
app.post(`/${API_PATH}/${API_VERSION}/generate-presigned-urls`, async (req, res) => {
    ...
    try {
      ...
      const parts = Math.ceil(fileSize / (100 * 1024 * 1024)); // 5MB per part (AWS S3 limit)
      const presignedUrls = [];
  ...
  // Route to abort the multipart upload (optional)
  app.post(`/${API_PATH}/${API_VERSION}/abort-upload`, async (req, res) => {
    ...
    try {
      ...
      res.send("Upload aborted successfully");
    } catch (error) {
      console.error(error);
